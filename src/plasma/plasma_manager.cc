...
void send_queued_request(event_loop *loop,
  ...
  int err = 0;
  switch (buf->type) {
  case MessageType_PlasmaQueueRemoteSubscribeRequest:
    err = handle_sigpipe(
...
void request_transfer_from(PlasmaManagerState *manager_state,
  ...
  if (manager_conn != NULL) {
    ...
    transfer_request->type = fetch_req->is_queue ? 
      MessageType_PlasmaQueueRemoteSubscribeRequest : MessageType_PlasmaDataRequest;
...
void process_message(event_loop *loop,
  ...
  case DISCONNECT_CLIENT: {
    RAY_LOG(DEBUG) << "[PlasmaManager] Disconnecting client on fd " << client_sock;
    event_loop_remove_file(loop, client_sock);
    ClientConnection_free(conn);
  } break;
  default:
